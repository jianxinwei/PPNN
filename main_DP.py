...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        for u in users:
            train_loader = prngDataloader(train_attributes, train_labels, dict_clients[0],
                                          batchsize=args.local_bs, gene=generator)  # args.batch_size
            local = LocalDPUpdate(args=args, train_loader=train_loader, clientID=u)
    ...
    torch.save(net_glob, './models/net_DP_bank.pkl')
    ...
    plt.xlabel('epoch')
    plt.savefig('./save/bankFL_DP_{}_{}.png'.format(args.model, args.epochs))

    # testing
    train_loader = DataLoader(dataset=TensorDataset(train_attributes, train_labels), batch_size=args.bs,
                              shuffle=True)
    # net_glob = torch.load('./models/net_DP_bank.pkl')
    net_glob.eval()
