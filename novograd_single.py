...
if __name__ == '__main__':
    ...
    optimizer = Novograd(net_glob.parameters(), lr=args.lr)
    ...
    with memory_time_moniter() as mt:
        for iter in range(args.epochs):
            ...
            if tmp_loss_valid < best_valid_loss:
                ...
                torch.save(net_glob, './save/novograd_single_best_{}.pkl'.format(args.dataset))
                print('SAVE BEST MODEL AT EPOCH {}'.format(iter))
            net_glob.train()

    torch.save(net_glob, './save/novograd_single_final_{}.pkl'.format(args.dataset))
    ...
    plt.grid(True)
    plt.legend(loc=0)
    plt.savefig('./save/{}_novograd_single_{}_{}.png'.format(args.dataset, args.model, args.epochs))

    # testing
    net_glob = torch.load('./save/novograd_single_best_{}.pkl'.format(args.dataset))
    net_glob.eval()
    if args.gpu != -1:
        test_loader = DataLoader(dataset=TensorDataset(test_attributes, test_labels), batch_size=args.bs, shuffle=True)
