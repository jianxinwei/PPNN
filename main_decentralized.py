...
if __name__ == '__main__':
    ...
    with timer() as t:
        for iter in range(args.epochs):
            ...
            if tmp_loss_valid < best_valid_loss:
                ...
                torch.save(net_glob, './save/decentralized_best_{}.pkl'.format(args.dataset))
    ...
    torch.save(net_glob, './save/decentralized_final_{}.pkl'.format(args.dataset))
    ...
    plt.grid(True)
    plt.legend(loc=0)
    plt.savefig('./save/{}_decentralized_{}_{}.png'.format(args.dataset, args.model, args.epochs))

    # testing
    train_loader = DataLoader(dataset=TensorDataset(train_attributes, train_labels), batch_size=args.bs, shuffle=True)
    net_glob = torch.load('./save/decentralized_best_{}.pkl'.format(args.dataset))
    net_glob.eval()
