...
from tphe.shamir_secret_sharing import ShamirSecretSharingScheme as Shamir
from tphe.shamir_secret_sharing_integers import ShamirSecretSharingIntegers as IntegerShamir
from tphe.shamir_secret_sharing_integers import mult_list
...
def generate_shared_paillier_key(keyLength=DEFAULT_KEYSIZE, n=NUMBER_PLAYERS, t=CORRUPTION_THRESHOLD,
                                 threshold=PRIME_THRESHOLD, it=MAX_ITERATIONS,
...
class PaillierSharedKey():
    ...
    def decrypt(self, Ciphertext, n, t, PublicKey, SecretKeyShares, theta):
        ...
        lagrange_interpol_enum = {i: mult_list([j for j in reconstruction_shares.keys() if i != j]) for i in
                                  reconstruction_shares.keys()}
        lagrange_interpol_denom = {i: mult_list([(j - i) for j in reconstruction_shares.keys() if i != j]) for i in
                                   reconstruction_shares.keys()}
        exponents = [(nFac * lagrange_interpol_enum[i] * reconstruction_shares[i]) // lagrange_interpol_denom[i] for i
                     in reconstruction_shares.keys()]
...
def is_biprime_parametrized(N, pShares, qShares, testValue):
    ...
    values = [int(powmod(testValue, x // 4, N)) for x in
              [sum(y) for y in zip(list(pShares.values())[1:], list(qShares.values())[1:])]]
...
if __name__ == "__main__":
    ...
    for length in lengths:
        ...
        print('SecretKeyShares', SecretKeyShares.shares)
