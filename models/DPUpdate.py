...
class LocalDPUpdate(object):
    def __init__(self, args, clientID=None):
        self.args = args
    ...
    def train(self, net, ldr_train):
        ...
        privacy_engine = PrivacyEngine(net, batch_size=self.args.local_bs, sample_size=len(ldr_train),
        ...
        for i in range(self.args.local_ep):
            ...
            for batch_idx, (attributes, labels) in enumerate(ldr_train):
                ...
                # if ((i + 1) % virtual_batch_rate == 0) or ((i + 1) == len(ldr_train)):
                #     optimizer.step()
                #     optimizer.zero_grad()
                # else:
                ...
                if self.args.verbose and batch_idx % 10 == 0:
                    print('Update Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                        i, batch_idx * len(attributes), len(ldr_train.dataset),
                           100. * batch_idx / len(ldr_train), loss.item()))
                batch_loss.append(loss.item())

                '''
                if batch_idx % self.args.pb_interval == 0:
